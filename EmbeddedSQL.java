...
public class EmbeddedSQL {
   ...
   public static void Query1(EmbeddedSQL esql){
      try{
	String query = "SELECT NUMBER(*), S.sname FROM Suppliers S, Parts P, Catalog C WHERE P.pid = C.pid AND S.sid = C.sid GROUP BY S.sid;";
	int rowCount = esql.executeQuery(query);
	System.out.println("total row(s): " + rowCount);
 	}catch(Exection e) {
	  System.err.println (e.getMessage());
   ...
   public static void Query2(EmbeddedSQL esql){
     try{
 	String query = "SELECT NUMBER(*), S.sname FROM Suppliers S, Parts P, Catalog C WHERE P.pid = C.pid AND S.sid = C.sid GROUP BY S.sid HAVING COUNT(*) > 2;";
   ...
   public static void Query6(EmbeddedSQL esql){
      try{
   String query = "SELECT UNIQUE S.address FROM Suppliers S, Parts P, Catalog C WHERE S.sid = C.sid AND P.pid = C.pid AND p.pname = ";
